#script que acessa o arquivo bd_rate.csv e cria gráfico com os bdrate para cada módulo

import matplotlib.pyplot as plt
import numpy as np 

def codigo_grafico1(pasta_tabelas, erros_SRAM): #Gráfico BD_Rate
    if erros_SRAM is True:
        erros = ['0.6V', '0.7V', '0.55V']   # eixo x
    else:
        erros = ['10-2','10-3','10-4','10-5','10-6','10-7']  # eixo x
    ...
    for linha in tabela3:   
        if(primeira_linha==1): #Linha 1 não precisa ser lida
            ...
            if (dados[4]!='0' and dados[4]!='0.0'): #não pegar dados quando o erro = 0
...
def monta_grafico(erros,bd_br,dados,pasta_tabelas, video, matriz): #Montando Gráfico
    ...
    elif(dados == "neighborintra_approx"):
    ...
    for linha in matriz: 
        if(linha[0] == dados and linha[2] == videoCompleto):
            if(linha[3] == "10E_02"):
                plt.scatter("10-2", float(linha[5]), c='blue')
            if(linha[3] == "10E_03"):
    ...
    plt.yticks(np.arange(-0.2, 0.65,0.1)) 
